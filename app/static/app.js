...
const App = (() => {
    ...
    function applyTheme(theme) {
        ...
        if (meta) meta.setAttribute('content', theme === 'dark' ? '#0B1220' : '#002D62');
    ...
    function renderDashboard(d) {
        ...
        if (meterNumEl) meterNumEl.textContent = d.meter_number || (dashboardData?.meter_number) || '--';
        ...
        if (usageSub) {
            usageSub.textContent = d.usage_rate_mode === 'manual'
        ...
        if (syncEl) {
            syncEl.textContent = d.last_scrape_at ? 'Updated ' + formatDateShort(new Date(d.last_scrape_at)) : 'Live';
        }
        ...
        if (d.last_token) {
            ...
            lastTokenEl.innerHTML = '<div class="empty-state"><p>No tokens recorded yet. Tap Fetch above or wait for background sync.</p></div>';
        }
    }
    ...
    function renderTokenItem(t, showPayer = false) {
        ...
        const sourceHtml = `<div class="token-source">${escapeHtml(t.source || '')}</div>`;
        const payerEditBtn = showPayer ?
            `<button class="btn btn-ghost btn-sm" style="padding:2px 6px;font-size:0.65rem;margin-top:4px" onclick="App.editPayerLabel(${t.id}, '${escapeHtml(t.payer_label || '')}')">${t.payer_label ? 'Edit label' : '+ Label'}</button>` : '';
        ...
        return `<div class="token-item">
            <div class="token-info">
                ...
                <div class="token-amount">${t.amount != null ? 'KES ' + t.amount.toFixed(0) : ''} ${escapeHtml(t.payment_mode || '')}</div>
                ${payerEditBtn}
            </div>
    ...
    function renderChart(snapshots) {
        ...
        usageChart = new Chart(canvas, {
            ...
            options: {
                ...
                scales: {
                    x: {
                        ticks: { color: c.tick, font: { size: 10, family: 'Inter' }, maxTicksLimit: 6 },
    ...
    function toastIcon(type) {
        if (type === 'success') return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>';
        if (type === 'error') return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>';
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (rateToggle) {
            rateToggle.addEventListener('keydown', e => {
                if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleRateMode(); }
            });
        }
    });
