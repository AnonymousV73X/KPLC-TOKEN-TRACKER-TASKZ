...
const App = (() => {
  ...
  function renderDashboard(d) {
    ...
    if (d.days_left != null) {
      ...
      daysCard.className =
        "stat-card " +
        (d.days_left < 2 ? "critical" : d.days_left < 5 ? "warning" : "ok");
    } else {
    ...
    if (d.last_token) {
      ...
      lastTokenEl.innerHTML =
        '<div class="empty-state" style="padding:20px"><p style="font-size:0.85rem;color:var(--text-muted)">No tokens recorded yet. Tap Fetch KPLC above or wait for background sync.</p></div>';
    }
  }
  ...
  async function loadTokenHistory(reset = false) {
    ...
    try {
      ...
      document.getElementById("token-count").textContent =
        (dashboardData?.total_tokens_count || 0) + " total";
  ...
  function renderTokenItem(t, showPayer = false) {
    ...
    const payerHtml = t.payer_label
      ? `<div class="token-payer">${escapeHtml(t.payer_label)}</div>`
      : "";
